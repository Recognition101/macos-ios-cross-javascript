...
const markedTemplates = require('./marked-templates.js');
...
/**
 * Gets the index of the first non-commented open-brace character.
 * @param {string} string the string containing an open-brace object
 * @return {number} the first non-commented open-brace character, or -1 if none.
 */
const getJsonStart = string => {
    let inLineComment = false;
    let inBlockComment = false;

    for(let i = 0; i < string.length; i += 1) {
        const previous = i === 0 ? '' : string.charAt(i - 1);
        const current = string.charAt(i);

        const isLineStart = previous === '/' && current === '/';
        const isLineEnd = current === '\n' || current === '\r';
        const isBlockStart = previous === '/' && current === '*';
        const isBlockEnd = previous === '*' && current === '/';
        inBlockComment = inBlockComment ? !isBlockEnd : isBlockStart;
        inLineComment = inLineComment ? !isLineEnd : isLineStart;

        if (current === '{' && !inBlockComment && !inLineComment) {
            return i;
        }
    }
...
const jsonParse = (json, adaptJs) => {
    if (!json) { return null; }
    const index = adaptJs ? getJsonStart(json) : 0;
    const jsonString = index > 0 ? json.substring(index) : json;
 ...
 * @return {ObjectMap<boolean|string>} a map of argument keys to values
 */
const parseArgs = () => {
...
module.exports = {
     ...
     */
    getInput: async (argStructure) => {
        const args = parseArgs();

        if (args['h'] || args['help']) {
            const args = argStructure.args;
            const argNames = args.map(a => `  --${a.name}, -${a.shortName}`);
            const maxNameLength = Math.max(...argNames.map(x => x.length)) + 4;

            const optsString = argStructure.args.map((arg, i) => {
        ...
        for(const arg of argStructure.args) {
            const value = args[arg.name] || args[arg.shortName];
    ...
    compile: async (appPath) => {
        ...
        const replacedHtml = html.replace(/<(script|link)[^>]*>/g, tag => {
            ...
            if (src) {
                const resText = fs.readFileSync(path.join(appPath, src));
                return tag.replace(srcRegex, `data-name="${src}"`) + '\n' +
                    resText + '\n';
            }
            if (isStyle && href) {
                const resText = fs.readFileSync(path.join(appPath, href));
                return '<style>\n' + resText + '\n</style>';
            }
            return tag;
        });
     ...
     * @param {string} imagePath the local path to store the image at
     * @return {Promise<number>} resolves to the status code for the request
     */
    downloadImage: async (url, imagePath) => new Promise(yes => {
        const get = url.startsWith('https') ? https.get : http.get;
        get(url, response => {
            const statusCode = (response.statusCode) || 0;
            if (statusCode === 200) {
                const stream = fs.createWriteStream(resolvePath(imagePath));
     ...
     */
    writeText: async (textPath, text) => new Promise((yes, no) => {
        fs.writeFile(resolvePath(textPath), text, e => e ? no(e) : yes());
    }),
     ...
     * @return {Promise<void>} a promise resolving after the file is written
     */
    writeJson: async (jsonPath, json) =>
        new Promise((yes, no) => {
            fs.writeFile(
                resolvePath(jsonPath),
     ...
     * @return {Promise<void>} a promise resolving after the file is written
     */
    writeBytes: async (bytesPath, bytes) => 
        new Promise(yes => {
            fs.writeFile(
     ...
     */
    output: (title, data) => { console.log(data + space); title; },
     ...
     */
    markdownToHtml: str => marked(str),

    /**
     * Styles that make a document appear as a GitHub markdown document.
     * @type {string}
     */
    markedCss: markedTemplates.css,

    /**
     * Creates an HTML document, given body HTML and optional CSS styles.
     * @param {string} body the string to use as the `<body>` tag's inner HTML
     * @param {string|null|undefined} [cssOverride] CSS text to inject. If
     *  not given, default GitHub markdown styles will be injected.
     * @return {string} the full HTML page string
     */
    markedHtml: markedTemplates.html
};
