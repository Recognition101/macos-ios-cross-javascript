...
module.exports = {
    ...
    getInput: async (argStructure, cacheArgs) => {
        ...
        const argsJsonString = string(args['shortcuts-args']);
        if (argsJsonString) {
            const argsJson = jsonParse(argsJsonString) || {};
            for(const [k, v] of Object.entries(argsJson)) {
        ...
        const isGetArgs = args['shortcuts-mode'] === 'shortcuts.getArgs';
        if (cacheArgs || args['cache-args'] || isGetArgs) {
            await cacheArgStructure(argStructure);
        }
        if (args['cache-args'] || isGetArgs) {
            const message = args['cache-args']
                ? `Arguments cached for: ${argStructure.name}`
                : JSON.stringify(argStructure, null, '    ');
            console.log(message);
            return null;
        }
