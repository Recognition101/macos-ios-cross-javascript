...
const helpUpdate = `Updates all item metadata in a given list.${helpFootnote}`;

/**
 * Gets the arguments for the App Lists Update command.
 * @param {AppLists} listSet the existing lists
 * @return {ArgStructure} the arguments for the update sales command
 */
const getArgumentsUpdate = (listSet) => {
    const listNames = Object.keys(listSet.lists);
    return {
        name: 'App Lists Update',
        inScriptable: true,
        help: helpUpdate,
        args: [{
            name: 'list',
            shortName: 'l',
            type: 'enum',
            help: 'The list whose items we will update.',
            choices: listNames.map(title => ({ title, code: title }))
        }]
    };
};
...
const writeLists = async (listSet, didUpdateListSet) => {
    ...
    if (didUpdateListSet) {
        ...
        await cacheArgStructure(getArgumentsUpdate(listSet));
    }
};
