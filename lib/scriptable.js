...
/** @type {import("./marked")} */
const marked = importModule('./marked.js');
/** @type {import("./marked-templates")} */
const markedTemplates = importModule('./marked-templates.js');
 ...
 */
const html = (statusText, logText) => markedTemplates.html(
...
module.exports.compile = async (appPath) => {
    ...
    const loadFile = path => {
        ...
        const popup = new Alert();
        popup.title = 'Error';
        popup.message = 'Could Not Load: ' + path;
        popup.presentAlert();
        return '';
    };
    ...
    const transformedHtml = html.replace(/<(script|link)[^>]*>/g, tag => {
        ...
        if (src) {
            ...
            const resText = loadFile(resPath);
 ...
 */
module.exports.downloadImage = async (url, imagePath) => {
    const imageReq = new Request(url);
    const image = await imageReq.load();
    const statusCode = Number(imageReq.response['statusCode']) || 0;
    if (statusCode === 200) {
        FileManager.local().write(resolvePath(imagePath), image);
    }
    return statusCode;
};
 ...
 */
module.exports.markdownToHtml = str => marked(str);
// @ts-ignore
module.exports.markedCss = markedTemplates.css;
module.exports.markedHtml = markedTemplates.html;
